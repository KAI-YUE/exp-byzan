...
class NaiveMLP(nn.Module):
    def __init__(self, in_dims, in_channels=1, out_dims=10, dim_hidden=100):
        super(NaiveMLP, self).__init__()
        self.fc1 = nn.Linear(in_dims, dim_hidden, bias=False)
        self.fc2 = nn.Linear(dim_hidden, out_dims, bias=False)
        # self.fc3 = nn.Linear(dim_hidden, out_dims)

        # self.fc2.weight.requires_grad = False
        # nn.init.uniform(self.fc2.weight.data, -1., 1)
        # self.fc2.weight.data = torch.where(self.fc2.weight.data>0, 1., -1.)
...
class NaiveMLP_binary(nn.Module):
    def __init__(self, in_dims, in_channels=1, dim_hidden=200):
        super(NaiveMLP_binary, self).__init__()
...
class VGG_7_GN(nn.Module):
    def __init__(self, in_dims, in_channels, out_dims=10):
        super(VGG_7_GN, self).__init__()
        self.in_channels = in_channels
        self.input_size = int(np.sqrt(in_dims/in_channels))
        self.fc_input_size = int(self.input_size/8)**2 * 512
        
        self.conv1_1 = nn.Conv2d(self.in_channels, 128, kernel_size=3, padding=1)
        self.gn1_1 = nn.GroupNorm(16, 128, affine=False)
        self.conv1_2 = nn.Conv2d(128, 128, kernel_size=3, padding=1)
        self.gn1_2 = nn.GroupNorm(16, 128, affine=False)
        self.mp1 = nn.MaxPool2d(kernel_size=2, stride=2)

        self.conv2_1 = nn.Conv2d(128, 256, kernel_size=3, padding=1)
        self.gn2_1 = nn.GroupNorm(32, 256, affine=False)
        self.conv2_2 = nn.Conv2d(256, 256, kernel_size=3, padding=1)
        self.gn2_2 = nn.GroupNorm(32, 256, affine=False)
        self.mp2 = nn.MaxPool2d(kernel_size=2, stride=2)

        self.conv3_1 = nn.Conv2d(256, 512, kernel_size=3, padding=1)
        self.gn3_1 = nn.GroupNorm(32, 512, affine=False)
        self.conv3_2 = nn.Conv2d(512, 512, kernel_size=3, padding=1)
        self.gn3_2 = nn.GroupNorm(32, 512, affine=False)

        self.mp3 = nn.AdaptiveMaxPool2d(2)
        self.fc = nn.Linear(2048, out_dims)
...
class VGG_7_BN(nn.Module):
    def __init__(self, in_dims, in_channels, out_dims=10):
        super(VGG_7_BN, self).__init__()
        self.in_channels = in_channels
        self.input_size = int(np.sqrt(in_dims/in_channels))
        self.fc_input_size = int(self.input_size/8)**2 * 512
